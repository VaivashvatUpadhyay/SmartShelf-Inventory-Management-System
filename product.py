...
class productClass:
    def __init__(self, root):
        ...
        title=Label(product_Frame,text="Manage Product Detials",font=("goudy old style",18),bg="#0f4d7d",fg="white").pack(side=TOP,fill=X)
        ...
        btn_clear=Button(product_Frame,text="Clear",command=self.clear,font=("goudy old style",15),bg="#607dab",fg="white",cursor="hand2").place(x=340,y=400,width=100,height=40)
    ...
    def update(self):
        ...
        try:
            if self.var_pid.get()=="":
                ...
                cur.execute("Select * from producg where pid=?", (self.var_pid_id.get(),))
                row=cur.fetchone()
                if row==None:
        ...
        try:
            if self.var_pid.get()=="":
                ...
                if row==None:
                    ...
                    op=messagebox.askyesno("Confirm","Do you relly want to delete?",parent=self.root)
                    if op==True:
    ...
    def search(self):
        ...
        try:
            if self.var_searchby.get()=="Select":
                messagebox.showerror("Error", "Select 'Search By' otpion",parent=self.root)
